...
function isVideoDone(status?: string) {
...
export default function Studio() {
  ...
  const videoSrc = isVideoDone(videoResult?.status) ? getVideoUrl(videoResult) || (videoId ? `/api/videos/${videoId}/content` : "") : "";
  ...
  function restoreHistory(item: HistoryItem) {
    ...
    if (item.videoId && !isVideoDone(restored.status) && !isVideoFailed(restored.status)) {
  ...
  async function generateImage() {
    ...
    try {
      if (referenceFiles.length > 0) {
        throw new Error(language === "zh" ? "图片参考图编辑暂时关闭，先用纯提示词生成图片。" : "Image reference editing is temporarily disabled. Use prompt-only image generation for now.");
      }
      ...
      <div className="workspace">
        ...
        <aside className="result-panel">
          <div className="result-head">
            ...
            {videoSrc ? (
              <a className="secondary-button" download href={videoSrc}><Download size={16} />{t.download}</a>
            ) : null}
          </div>
          ...
          <div className="preview">
            ...
            ) : videoResult && !isVideoDone(videoResult.status) ? (
            ...
            <div className="batch-results">
              {batchJobs.map((job, index) => (
                <div className="batch-job" key={job.id}>
                  ...
                  {job.url ? <a className="secondary-button" href={job.url} target="_blank" rel="noreferrer"><Download size={15} />{t.download}</a> : null}
